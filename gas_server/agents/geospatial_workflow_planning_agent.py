...
class GeospatialWorkflowPlanningAgent(GeoAgent):
    ...
    def _write_python_workflow(self, query: str, plan: dict[str, Any]) -> str:
        ...
        code = [
            ...
            "",
            "DEFAULT_CREDENTIALS = {",
            "    # Inspect each agent's DescribeAgent JSON and add the credential",
            "    # field names expected by that server/agent, for example:",
...
function buildOperationCode(node, step) {{
  ...
  return [
    ...
    '',
    'DEFAULT_CREDENTIALS = {{',
    '    # Inspect DescribeAgent and add required server/agent credential keys here.',
    "    # 'OPENAI_API_KEY': 'YOUR_OPENAI_API_KEY',",
    "    # 'GEMINI_API_KEY': 'YOUR_GEMINI_API_KEY',",
    '}}',
    '',
